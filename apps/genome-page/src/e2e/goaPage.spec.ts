...
test("Renders Molecular Function panel", async ({ page }) => {
  ...
  await expect(page.getByText("2025-09-15").first()).toBeVisible()
  await expect(
...
test("Sorts data when property in the header is clicked", async ({ page }) => {
  ...
  await expect(firstMolecularFunctionRow.getByText("2025-09-15")).toBeVisible()
