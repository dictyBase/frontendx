...
describe('NewsItem', () => {
  const renderNewsItem = () => {
    const router = createMemoryRouter([
      {
        index: true,
        element: <NewsItem {...newsItemProps} />
      }
    ])
    render(
  ...
  test('renders a link to `/news/${name}/show`', () => {
    renderNewsItem()
    expect(screen.getByRole('link', { name: /read more/i })).toHaveAttribute('href', `/news/${newsItemProps.name}/show`)
    expect(screen.getByText(/Annual International Dictyostelium Conference/)).toBeInTheDocument()
  })
})